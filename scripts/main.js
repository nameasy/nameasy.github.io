...
;(function(window) {
	...
	function GridFx(el, options) {
		...
		this.items = [].slice.call(this.gridEl.querySelectorAll('.gallery__item'));
		this.previewEl = nextSibling(this.gridEl);
		this.isExpanded = false;
		this.isAnimating = false;
		this.closeCtrl = this.previewEl.querySelector('.preview__close');
		this.previewDescriptionEl = this.previewEl.querySelector('.preview__description');
	...
	GridFx.prototype._init = function() {
		...
		imagesLoaded( this.gridEl, function() {
			...
			classie.add(self.gridEl, 'gallery__list--loaded');
			// init/bind events
			self._initEvents();
	...
	GridFx.prototype._initEvents = function () {
		...
		this.items.forEach(function(item) {
			...
			item.querySelector('.gallery__link').addEventListener(clickEvent, function(ev) {
				if(clickEvent === 'click') {
					ev.preventDefault();
					self._openItem(ev, item);
				}
	...
	GridFx.prototype._openItem = function(ev, item) {
		...
		// item's image
		var gridImg = item.querySelector('.gallery__image'),
		...
		this._setOriginal(item.querySelector('.gallery__link').getAttribute('href'));
		...
		classie.add(item, 'gallery__item--is-current');
		...
		var descriptionEl = item.querySelector('.gallery__figcaption');
		if( descriptionEl ) {
		...
		setTimeout(function() { 
			...
			classie.add(self.previewEl, 'preview--is-open');
			// callback
	...
	GridFx.prototype._setOriginal = function(src) {
		if( !src ) {
			...
			this.originalImg.className = 'preview__original';
	...
	GridFx.prototype._setClone = function(src, settings) {
		if( !src ) {
			...
			this.cloneImg.className = 'preview__clone';
			src = '';
	...
	GridFx.prototype._closeItem = function() {
		...
		var gridItem = this.items[this.current],
			gridImg = gridItem.querySelector('.gallery__image'),
			gridImgOffset = gridImg.getBoundingClientRect(),
			self = this;

		classie.remove(this.previewEl, 'preview--is-open');
		...
		classie.add(this.originalImg, 'preview__original--animate');
		...
		onEndTransition(this.originalImg, function() {
			...
			classie.remove(gridItem, 'gallery__item--is-current');
			...
			onEndTransition(self.originalImg, function() {
				...
				classie.remove(self.originalImg, 'preview__original--animate');
